...
const DIET_PRINCIPLES = [
  ...
  { label: "배달 X",      bad: true  },
  { label: "기름 X",      bad: true  },
  { label: "식이섬유 ✓",  bad: false },
  { label: "중탄고단저지 ✓", bad: false },
  { label: "물 많이 ✓",   bad: false },
];
...
function loadChecks() {
  try { const r = localStorage.getItem("rt-checks-v2"); return r ? JSON.parse(r) : {}; } catch { return {}; }
}
function saveChecks(d) { try { localStorage.setItem("rt-checks-v2", JSON.stringify(d)); } catch {} }
function loadRoutines() {
  try { const r = localStorage.getItem("rt-routines-v2"); return r ? JSON.parse(r) : DEFAULT_ROUTINES; } catch { return DEFAULT_ROUTINES; }
}
function saveRoutines(r) { try { localStorage.setItem("rt-routines-v2", JSON.stringify(r)); } catch {} }
...
const iStyle = {
  ...
  borderRadius: 8, padding: "9px 12px", color: "#1E293B", fontSize: 13,
...
export default function App() {
  ...
  useEffect(() => { setChecks(loadChecks()[today] || {}); }, []);

  function toggle(id) {
    const u = { ...checks, [id]: !checks[id] };
    setChecks(u);
    const all = loadChecks(); all[today] = u; saveChecks(all);
  }
  function deleteItem(dayKey, id) {
    const u = { ...routines, [dayKey]: (routines[dayKey] || []).filter(i => i.id !== id) };
    setRoutines(u); saveRoutines(u);
  }
  function startEdit(dayKey, item) {
    setEditingId(item.id);
    setForm({ label: item.label, category: item.category, note: item.note, time: item.time });
  }
  function saveEdit() {
    const u = { ...routines, [editDay]: (routines[editDay] || []).map(i => i.id === editingId ? { ...i, ...form } : i) };
    setRoutines(u); saveRoutines(u); setEditingId(null);
  }
  function addItem(dayKey) {
    if (!form.label.trim()) return;
    const id = `c-${dayKey}-${Date.now()}`;
    const u = { ...routines, [dayKey]: [...(routines[dayKey] || []), { ...form, id }] };
    setRoutines(u); saveRoutines(u); setAddingDay(null);
    setForm({ label: "", category: "공부", note: "", time: "" });
  }
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#F1F5F9" }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ background: "#fff", borderBottom: "1px solid #E2E8F0", padding: "env(safe-area-inset-top, 12px) 20px 0", paddingTop: "max(env(safe-area-inset-top, 0px), 16px)" }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                        {DIET_PRINCIPLES.map(tag => (
                          <span key={tag.label} style={{
                            ...
                            color: tag.bad ? "#B45309" : "#065F46",
                            background: tag.bad ? "#FEF3C7" : "#D1FAE5",
                            border: `1px solid ${tag.bad ? "#FCD34D" : "#6EE7B7"}`,
                  ...
                  {items.map(item => {
                    ...
                    return (
                      <div key={item.id} onClick={() => toggle(item.id)} style={{
                        ...
                        borderLeft: `3px solid ${checked ? "#CBD5E1" : meta.color}`,
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {(item.time || item.note) && (
                            <div style={{ display: "flex", gap: 6, marginTop: 2, alignItems: "center" }}>
                              {item.time && (
                                <span style={{ fontSize: 11, fontWeight: 600, color: meta.color, background: meta.bg, borderRadius: 4, padding: "1px 6px" }}>{item.time}</span>
                              )}
          ...
          <div>
            <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
              {[["weekday", "평일 공통"], ...DAY_KEYS.map((k, i) => [k, DAYS[i]])].map(([k, label]) => (
                <button key={k} onClick={() => { setEditDay(k); setEditingId(null); setAddingDay(null); }} style={{
                  background: editDay === k ? "#0F172A" : "#fff",
                  border: `1px solid ${editDay === k ? "#0F172A" : "#E2E8F0"}`,
                  color: editDay === k ? "#fff" : "#64748B",
                  borderRadius: 20, padding: "5px 12px", fontSize: 12,
                  fontWeight: editDay === k ? 600 : 400, cursor: "pointer", transition: "all 0.15s",
                }}>{label}</button>
              ))}
            </div>
            ...
            <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, overflow: "hidden", marginBottom: 8 }}>
              {(routines[editDay] || []).length === 0 && (
                <div style={{ padding: "20px", textAlign: "center", color: "#94A3B8", fontSize: 13 }}>항목이 없어요</div>
              )}
            ...
            {addingDay === editDay ? (
              ...
              <button onClick={() => { setAddingDay(editDay); setEditingId(null); setForm({ label: "", category: "공부", note: "", time: "" }); }} style={{
                width: "100%", background: "#fff", border: "1.5px dashed #CBD5E1", borderRadius: 12,
                color: "#94A3B8", padding: "12px", cursor: "pointer", fontSize: 13, transition: "border-color 0.15s",
              }}>+ 항목 추가</button>
            )}
          </div>
        )}
